...
import { ThumbsUp, ThumbsDown, Plus, Flag, MessageSquare, Heart, Clock, Trash2, Share2, Check } from 'lucide-react';
      ...
      if (data) {
        ...
        if (!sessionStorage.getItem(`viewed_${id}`)) {
          viewTimer = setTimeout(() => {
            ...
            // Optimistically update view count locally for immediate feedback
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 py-3 border-y border-[#2a2a2a]">
             <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 w-full sm:w-auto">
                ...
                <button 
                 onClick={handleShare}
                 className={`flex items-center gap-2 px-6 py-2 rounded-full transition-colors bg-[#161616] text-gray-300 hover:bg-[#2a2a2a]`}
                >
                ...
                {visibleRecommended.map(v => (
                   <div key={v.id} className="flex flex-col gap-2 group hover:bg-[#161616] p-2 rounded-xl border border-transparent hover:border-[#2a2a2a] transition-all">
                     ...
                     <div className="flex flex-col">
                        ...
                        <p className="text-[#aaa] text-xs pb-1 opacity-70">{formatViews(v.views)} views • {v.uploadedAt?.toDate?.().toLocaleDateString() || 'Recently'}</p>
                     </div>
                   </div>
                ))}
             </div>
