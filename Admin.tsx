...
const AdCard = ({ type, ad }: { type: string, ad: any }) => {
    ...
    <div className="bg-[#0a0a0a] border border-[#2a2a2a] p-4 rounded-lg flex flex-col gap-3">
      <div className="flex justify-between items-center whitespace-nowrap">
        ...
        <label className="flex items-center gap-2 cursor-pointer group">
          <span className="text-xs text-gray-400 group-hover:text-emerald-500 transition-colors">Always Active</span>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input type="text" className="flex-1 bg-[#161616] border border-[#2a2a2a] rounded text-white px-2 py-1 text-sm focus:outline-none focus:border-wine-500" value={imageUrl} onChange={e => setImageUrl(e.target.value)} />
          <button onClick={() => updateAd(type, { ...ad, imageUrl, active: true })} className="bg-[#2a2a2a] hover:bg-[#333] text-white px-3 py-1 rounded text-sm transition-colors"><Check size={16}/></button>
        </div>
        {imageUrl && (
           <div className="mt-2 w-full h-16 border border-[#2a2a2a] rounded overflow-hidden">
              <img src={imageUrl} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/320x100?text=Invalid+Image'; }} />
           </div>
        )}
      </div>
      <div>
        ...
        <div className="flex gap-2">
          <input type="text" className="flex-1 bg-[#161616] border border-[#2a2a2a] rounded text-white px-2 py-1 text-sm focus:outline-none focus:border-wine-500" value={targetUrl} onChange={e => setTargetUrl(e.target.value)} />
          <button onClick={() => updateAd(type, { ...ad, targetUrl, active: true })} className="bg-[#2a2a2a] hover:bg-[#333] text-white px-3 py-1 rounded text-sm transition-colors"><Check size={16}/></button>
        </div>
      </div>
    </div>
  );
  ...
  const handleEmbedChange = async (val: string) => {
    ...
    // Auto-fill description with title
    if (!vDescription) {
      setVDescription(extractedTitle || 'Enjoy this amazing video!');
    }
    ...
    if (extractedCatName) {
      ...
      if (existing) {
        ...
        const confirmCreate = window.confirm(`Detected new category: "${extractedCatName}". Would you like to create it?`);
        if (confirmCreate) {
          try {
  ...
  const handleUpload = async () => {
    ...
    if (vDescription) data.description = vDescription;
  ...
  const loadIntoForm = (video: VideoData) => {
    ...
    setVDescription(video.description || '');
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-[#161616] border border-[#2a2a2a] rounded-xl p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <label className="text-sm text-gray-400 mb-1 block">Video Description</label>
              <textarea 
                value={vDescription} onChange={e => setVDescription(e.target.value)}
                className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-wine-500 h-24 resize-none" 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <div className="bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg p-2 max-h-48 overflow-y-auto grid grid-cols-2 gap-2">
                    {actors.map(actor => {
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-center gap-2 p-1.5 rounded cursor-pointer border transition-all ${isSelected ? 'bg-wine-600/20 border-wine-500' : 'bg-[#161616] border-transparent hover:bg-[#222]'}`}
                        >
                          <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 border border-gray-700">
                            {actor.profile_photo ? (
                              ...
                              <div className="w-full h-full bg-gray-800 flex items-center justify-center text-[10px] font-bold text-white">
                                {actor.name.charAt(0).toUpperCase()}
                              </div>
                            )}
                          </div>
                          <span className="text-xs text-white truncate">{actor.name}</span>
                          {isSelected && <div className="ml-auto w-2 h-2 rounded-full bg-wine-500 shadow-[0_0_5px_#f00]"></div>}
                        </div>
                      );
                    })}
                  </div>
      ...
      <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-6 mt-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {['top_banner', 'bottom_banner', 'video_ad', 'popup_ad', 'desc_ad'].map(type => {
            const ad = ads.find(a => a.type === type) || { type, imageUrl: '', targetUrl: '', active: true };
                 ...
                 <tr key={video.id} className="border-b border-[#2a2a2a] hover:bg-[#2a2a2a]/50 transition-colors">
                   <td className="px-4 py-3">
                     <div className="flex items-center gap-3">
                       <img src={video.thumbnail || "https://via.placeholder.com/160x100?text=No+Thumb"} className="w-16 h-10 object-cover rounded" alt="" />
                   ...
                   <td className="px-4 py-3 flex items-center gap-2">
                     ...
                     <button onClick={() => deleteVideo(video.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-500/20 rounded transition-colors"><Trash2 size={16}/></button>
                   </td>
                 </tr>
               ))}
               {videos.length === 0 && (
